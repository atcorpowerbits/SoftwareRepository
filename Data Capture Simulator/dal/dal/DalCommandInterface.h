			...
			private ref class DalCommandInterface: public DalStatusHandler
			{
				...
				public:
					...
					*/
					...
					static bool TranslateUnsignedShort( array <unsigned char>^ destinationArray, int startPostion, unsigned short sourceUS);
					static bool TranslateUnsignedLong( array <unsigned char>^ destinationArray, int startPostion, unsigned long sourceUL );

					DalReturnValue SendDataCapturePacket(DalEM4Command^ serialCommand); 

					/**
					* Handler for to listen to PC commands
					* @param[in]	sender	Event sender. 
					* @param[in]	e	Event arguments
					*/
					static void PCCommandHandler(Object^ sender, SerialDataReceivedEventArgs^ e);
