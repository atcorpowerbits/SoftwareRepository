	...
	namespace Scor { 
		...
		namespace DataAccess{
				...
				*/
				...
				public ref class PCCommandRecievedEventArgs: public EventArgs
				{
					public:
						/**
						* Data for the event.
						*/
						property DalEM4Command ^pcCommandPacket;
						...
						* @param[in] cuffStateFlag	The data for the event (The resolved status flag).						
						*/
						PCCommandRecievedEventArgs(DalEM4Command^ pcCommandPacket);
				};

				public delegate void PCCommandRecievedEventHandler(Object^ sender, PCCommandRecievedEventArgs ^args);
				...
				*/
				public ref class DalEventContainer
				{
					...
					//	DalModuleErrorAlarmEventHandler^     _dalModuleErrorAlarmEventHandler;
						...
						PCCommandRecievedEventHandler^  _pcCommandRecievedEventHandler;
					...
					public:
						...
						*/
						...
						/**
						* PC command recieved event.
						*/
						event PCCommandRecievedEventHandler^ OnPcCommandRecievedEvent
						{
							/**
							* Registers specifed handler method as a listener to this event.
							*
							...
							*						Should match the signature of DalTonometerStatusEventHandler
							*/
							void add(PCCommandRecievedEventHandler^ handler)
							{
								lock lockEvents(this);
								//add the specified handler as listener.
								_pcCommandRecievedEventHandler += handler;
							}
							...
							*						Should be already added as a listener.
							*/
							void remove(PCCommandRecievedEventHandler^ handler)
							{
								lock lockEvents(this);
								//Remove the specified handler from the list of listeners
								_pcCommandRecievedEventHandler -= handler;
							...
							* @param[in[	args	The arguments for this event. Should be of the type DalTonometerStatusEventArgs.
							*/
							void raise(Object^ sender, PCCommandRecievedEventArgs ^args)
							{
								//Raise the event.
								if(_pcCommandRecievedEventHandler)
								{
									_pcCommandRecievedEventHandler->Invoke(sender, args);
									//AtCor::Scor::CrossCutting::Logging::CrxLogger::Instance->Write("Deepak>>>> Tonometer event raised in DAL: "+ args->CuffStateFlag.ToString() );
								}
							}
						}
