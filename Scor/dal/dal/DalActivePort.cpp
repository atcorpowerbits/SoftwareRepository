	...
	namespace Scor { 
		namespace DataAccess{
			...
			DalActivePort::DalActivePort()
			{
				...
				/*	readerThread = gcnew Thread(gcnew ThreadStart(this, &DalActivePort::ThreadMethod));*/
			...
			bool DalActivePort::SetSerialPortProperties()
			{
				...
				try
				{
					...
					//_serialPort->ReceivedBytesThreshold  = DalConstants::EM4ZeroDataResponsePacketSize + DalConstants::EM4NackPacketDataLength ;
			...
			void DalActivePort::SendPacket(array<unsigned char>^dataToSend)
			{
				try
				{
					...
					CrxLogger::Instance->Write("Deepak>>> DalActivePort::SendPacket Command sent on port:  " +  AtCor::Scor::DataAccess::DalStatusHandler::ConvertBytesToString(dataToSend));
				}
				catch(Exception^ excepObj)
				{
					CrxLogger::Instance->Write("Deepak>>> DalActivePort::SendPacket Failed to send:  " +  AtCor::Scor::DataAccess::DalStatusHandler::ConvertBytesToString(dataToSend));
					throw gcnew ScorException(excepObj);
				}
			}
