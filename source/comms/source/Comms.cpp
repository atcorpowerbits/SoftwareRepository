...
bool comms_open(COMMS_ID_TYPE port_id, bool disable_errors)
{
   bool    do_open = false;
   ...
   else
   {
      if (comms_port)
      {
         if (port_id == comms_port->ComNumber)
         {
            if (!comms_port->Open)
            {
               do_open = true;
         ...
         if (do_open)
         {
            comms_port->ComNumber = port_id;
            try
            {
               comms_port->Open = true;
               comms_port_opened = true;
            }
