...
*/
//---------------------------------------------------------------------------
bool TPressure::CalculateTrigPts(const int pAlgorithm, const int pMinTrigPt, bool singlePulse)
{
  // Validate the signal length. Not relevant for a single pulse
  if (!singlePulse)
  {
  ...
  // Find onset of pulses. Returns false if the number of onsets is less than the minimum
  if (FindOnsets(pAlgorithm, pMinTrigPt)==false)
  {
    // For a single pulse, the minimum number of onsets is acceptable
    if (singlePulse && (NofOnsets == PWA_MIN_NOF_ONSETS))
    {
      return true;
    }
...
*/
//---------------------------------------------------------------------------
bool TPressure::CalcPulses(const bool pRejectExtraPulses, const int pTail, bool singlePulse)
{
  if (singlePulse)
  {
      ...
      // Calculate the length of the last pulse
      ...
      // Ensure the trigger point is not too close to the start
