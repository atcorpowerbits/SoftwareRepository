...
*/
//---------------------------------------------------------------------------
bool TPWM::Calculate(bool append, bool singlePulse)
{
  bool ret;
  ...
  if (Calibrated && Continuous)
  {
    ...
    if (singlePulse)
    {
      ret = TPWA::CalculateSinglePulse(1.0);//CalibrationRatio);
    }
    else
    {
  ...
  else if (!Calibrated && Continuous)
  {
    ...
    if (singlePulse)
    {
      ret = TPWA::CalculateSinglePulse();//CalibrationRatio);
    }
    else
    {
      ret = TPWA::Calculate();
