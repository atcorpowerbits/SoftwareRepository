...
VAR_EXPORT  int        capture_successful_pulses[CAPTURE_MAX_PULSES];
VAR_EXPORT  int        capture_pulse_index;
VAR_EXPORT  int        capture_nof_pulses;
VAR_EXPORT  bool       capture_pulse_buff_full;
...
void capture_reset(MEAS_ID_TYPE meas_id)
{
   ...
   switch (meas_id)
   {
      ...
      case MEAS_PWM_ID:
         ...
         frm_capture_pwm->srs_rtof->Clear();
         ...
         */
         ...
         else
         {
            ...
            if (frm_capture_pwm->pwm->AIxSendOut)
            {
                frm_capture_pwm->ChartAI->BottomAxis->SetMinMax(0, CAPTURE_PWM_OUTPUT_REFRESH_RATE);
            }
            else
            {
...
/*
// Reset output parameter values
*/
void capture_reset_pwm_output(void)
{
   capture_pulse_buff_full = false;
   capture_pulse_index = 0;
   capture_nof_pulses = 0;

   for (int i = 0; i < CAPTURE_MAX_PULSES; i++)
   {
      capture_successful_pulses[i] = 0;
   }
}
   ...
   if (is_data_valid && (pressure_sample >= 0))
   {
      ...
      if (i == 4)
      {
         ...
         for (i = 0; i < 4; i++)
         {
            ...
            {
               // Calculate RTOF values for Radial measurements when the output AIx
               // option has been selected by the user
               if (frm_capture_pwm->pwm->AIxSendOut && frm_capture_pwm->pwm->MeasureType == PWA_RADIAL)
               {
                  // If the display needs updating
                  if (rtof_add_sample(data_buff[0][i]))
                  {
                     // If a pulse was found
                     if (rtof_get(&rtof) && rtof.upper_guidance_band != DEFAULT_VALUE)
                     {
                        // Display a line to indicate the pulse
                        frm_capture_pwm->srs_rtof->AddXY(time_val, 0, "", clTeal);

                        int capture_pulse_length;
                        int capture_pulse_previous_index;

                        // Calculate the length to the 2nd last successful pulse if it exists
                        if (capture_nof_pulses > 2)
                        {
                           // Find the 2nd last index in the circular buffer
                           if (capture_pulse_index > 1)
                           {
                              capture_pulse_previous_index = capture_successful_pulses[capture_pulse_index - 2];
                           }
                           else
                           {
                              capture_pulse_previous_index = capture_successful_pulses[CAPTURE_MAX_PULSES + capture_pulse_index - 2];
                           }
                           // Find the capture pulse length from the circular buffer indexes
                           if (capture_pulse_previous_index > capture_circbuff_index)
                           {
                              capture_pulse_length = (capture_circbuff_index + frm_capture_pwm->total_samples) - capture_pulse_previous_index;
                           }
                           else
                           {
                              capture_pulse_length = capture_circbuff_index - capture_pulse_previous_index;
                           }
                        }
                        // Otherwise capture the maximum pulse length
                        else
                        {
                           capture_pulse_length = PRESSURE_PULSE_MAXLENGTH;
                        }

                        pressure_data = new short int[capture_pulse_length];

                        // Start 1 pulse from the index of the circular buffer
                        int bufferIndex = capture_circbuff_index - capture_pulse_length;
                        int dataIndex = 0;       
                        if (bufferIndex < 0)
                        {
                           // Add points until you reach the end of the buffer
                           bufferIndex = bufferIndex + frm_capture_pwm->total_samples;
                           while (((dataIndex < capture_pulse_length) && (bufferIndex < frm_capture_pwm->total_samples)))
                           {
                              pressure_data[dataIndex++] = capture_ch1_buff[bufferIndex++];
                           }
                           // Then add points from the start of the buffer
                           bufferIndex = 0;
                           while (((dataIndex < capture_pulse_length) && (bufferIndex < capture_circbuff_index)))
                           {
                              pressure_data[dataIndex++] = capture_ch1_buff[bufferIndex++];
                           }
                        }
                        ...
                        {
                           // Add points from the buffer index
                           while (((dataIndex < capture_pulse_length) && (bufferIndex < capture_circbuff_index)))
                           {
                              pressure_data[dataIndex++] = capture_ch1_buff[bufferIndex++];
                           }
                        ...
                        /* Start 1 pulse from the end of the circular buffer
                        int bufferIndex = capture_circbuff_index + (frm_capture_pwm->total_samples - PRESSURE_PULSE_MAXLENGTH);
                        int dataIndex = 0;
                        // Add points until you reach the end of the buffer
                        while (((dataIndex < PRESSURE_PULSE_MAXLENGTH) && (bufferIndex < frm_capture_pwm->total_samples)))
                        {
                           pressure_data[dataIndex++] = capture_ch1_buff[bufferIndex++];
                        }
                        // Then add points from the start of the buffer
                        bufferIndex = 0;
                        while (((dataIndex < PRESSURE_PULSE_MAXLENGTH) && (bufferIndex < capture_circbuff_index)))
                        {
                           pressure_data[dataIndex++] = capture_ch1_buff[bufferIndex++];
                        } */
                        ...
                        while (create_report && dataIndex < capture_pulse_length)
                        {
                           if (pressure_data[dataIndex] < capture_raw_data_threshold)
                           {
                              create_report = false;
                           }
                           ++dataIndex;
                        }
                        if (create_report && math_MinMaxInArray(pressure_data, capture_pulse_length, signal_min, signal_max))
                        {
                        ...
                        if (create_report)
                        {
                           // Give Windows time to empty the message queue
                           Application->ProcessMessages();
                           // Initialse the signal classes
                           frm_capture_pwm->pwm->PrepareToCaptureSignal();
                           // Populate the signal class
                           frm_capture_pwm->pwm->Periph.CaptureSignal(pressure_data, capture_pulse_length);

                           // Calculate parameters for a single pulse
                           if (frm_capture_pwm->pwm->Calculate(false, true))
                           {
                              delete [] pressure_data;

                              // Store the index of successfully calculated pulses
                              capture_successful_pulses[capture_pulse_index] = capture_circbuff_index;

                              // Update the circular buffer
                              capture_pulse_index++;
                              capture_nof_pulses++;
                              if (!capture_pulse_buff_full)
                              {
                                 capture_pulse_buff_full = (capture_pulse_index >= CAPTURE_MAX_PULSES) ? true : false;
                              }
                              capture_pulse_index %= CAPTURE_MAX_PULSES;

                              // Display Augmentation Index
                              if (frm_capture_pwm->pwm->Central.AGPH >= DEFAULT_VALUE)
                              {
                                 ...
                                 frm_capture_pwm->ChartAI->Color = sphygmocor_red;
                              }
                              else
                              {
                                 ...
                                 // Output Augmentation Index
                                 send_parameter_data[0] = (INT8)math_Round(frm_capture_pwm->pwm->Central.AGPH);
                                 comms_send_parameter(send_parameter_data, 1);
                                 frm_capture_pwm->ChartAI->Color = sphygmocor_green;
                              }
                              ++frm_capture_pwm->trend_counter;
                              ++frm_capture_pwm->calculated_trend_counter;
                              if (frm_capture_pwm->trend_counter % CAPTURE_PWM_OUTPUT_REFRESH_RATE == 0)
                              {
                                 frm_capture_pwm->ChartAI->BottomAxis->SetMinMax(0, frm_capture_pwm->trend_counter + CAPTURE_PWM_OUTPUT_REFRESH_RATE);
                              }
                           }
                           else
                           {
                     ...
                     // RTOF has not detected a pulse for a period of time (2 seconds)
                     else
                     {
                        capture_reset_pwm_output();
                        frm_capture_pwm->ChartAI->Color = sphygmocor_red;
